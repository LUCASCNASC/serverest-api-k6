...
export const options = {
  vus: 1, // Smoke Test utiliza carga mínima (1 usuário virtual)
  duration: '10s',
  thresholds: {
...
export default function () {
  ...
  check(res, { 'User POST - 201 (Sucesso)': (r) => r.status === 201 });
  userId = res.json()._id;
  ...
  check(res, { 'Login POST - 401 (Não autorizado)': (r) => r.status === 401 });
  ...
  check(res, { 'Cart POST - 201 (Sucesso)': (r) => r.status === 201 });

  // Falha: Usuário já possui carrinho (400)
  res = http.post(`${BASE_URL}/carrinhos`, cartPayload, { headers: authHeaders });
  check(res, { 'Cart POST - 400 (Carrinho duplicado)': (r) => r.status === 400 });

  // --- 5. VALIDAÇÃO DE RESTRIÇÕES E LIMPEZA ---
  // Erro: Tentar excluir usuário com carrinho ativo (400)
  res = http.del(`${BASE_URL}/usuarios/${userId}`);
  check(res, { 'User DELETE - 400 (Bloqueio por carrinho)': (r) => r.status === 400 });

  // Concluir Compra / Excluir Carrinho (200)
  res = http.del(`${BASE_URL}/carrinhos/concluir-compra`, null, { headers: authHeaders });
  check(res, { 'Cart DELETE - 200 (Compra concluída)': (r) => r.status === 200 });

  // Agora é permitido excluir o usuário após concluir a compra
  res = http.del(`${BASE_URL}/usuarios/${userId}`);
  check(res, { 'User DELETE - 200 (Sucesso)': (r) => r.status === 200 });
